...
public:
    ...
    // the corresponding supervising data "y". The order of the data samples are
    ...
    virtual _COMP_T compute_obj(_COMP_T* dat, _N_DAT_T n, _SUPV_T* y) = 0;
    ...
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    dimension(_DAT_DIM_T _dimension) {
        if (dim) {
    ...
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    learning_rate_try_1st(_COMP_T _eta0_1st_try) {
        eta0_1st_try    = _eta0_1st_try; return *this;
    }
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    learning_rate_try_factor(_COMP_T _eta0_try_factor) {
        eta0_try_factor = _eta0_try_factor; return *this;
    }
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    compute_obj_each_epoch(bool _compute_obj) {
        comp_obj_each_epoch = _compute_obj; return *this;
    }
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    ...
    _DAT_DIM_T   dimension()                const { return dim; }
...
protected:
    ...
    SGD<_COMP_T, _SUPV_T, _DAT_DIM_T, _N_DAT_T>& 
    determine_eta0(_COMP_T* dat, _N_DAT_T n, _SUPV_T* y,
...
train(_COMP_T* dat, _N_DAT_T n, _SUPV_T* y) {
    ...
    if (!eta0) determine_eta0(dat, n, y);
    while (t < n_epoch * n) {
...
train_epoch(_COMP_T* dat, _N_DAT_T n,
    ...
    for (_N_DAT_T i = 0; i < m; ++i, ++t) {
        ...
        if (out_training_proc) {
            *out_training_proc << compute_obj(dat, n, y) << " ";
...
determine_eta0(_COMP_T* dat, _N_DAT_T n, _SUPV_T* y,
    ...
    for (_N_DAT_T i = 0; i < m; ++i) {
        ...
        tempSGD->train_one(dat, ind, n, y[ind]);
        if (verbosity >= 3) {
            std::cout << "Done." << std::endl;
        }
    }
